importScripts('webextension/browser-polyfill.js');
importScripts('llm-impact/llm-tracker.js');
...
function storeResult(url, parsedData) {
    ...
    chrome.storage.local.set({ [url]: {...parsedData, visitedAt } });
}

// Add a function to get stored results
...
browser.tabs.onUpdated.addListener(async (id, changeInfo, tab) => {
    if (tab.status == "complete" && tab.active) {
        ...
        const llmService = llmTracker.detectLLMService(tab.url);
        if (llmService) {
            ...
            if (scriptInjected) {
                ...
                const llmData = llmTracker.getCurrentInteraction(tab.url);
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "isLLMService") {
        const llmService = llmTracker.detectLLMService(message.url);
        return Promise.resolve(llmService !== null);
    }
    
    if (message.action === "getLLMData") {
        const data = llmTracker.getCurrentInteraction(message.url);
        return Promise.resolve(data);
    }
    
    if (message.action === "startLLMTracking") {
        llmTracker.startTracking(message.url, message.service);
        return Promise.resolve({status: "started"});
    }
    
    if (message.action === "updateLLMInput") {
        llmTracker.updateInputTokens(message.text);
        return Promise.resolve({status: "updated"});
    }
    
    if (message.action === "updateLLMOutput") {
        llmTracker.updateOutputTokens(message.text);
        return Promise.resolve({status: "updated"});
    }
    
    if (message.action === "completeLLMTracking") {
        if (message.inputText) {
            llmTracker.updateInputTokens(message.inputText);
        }
        if (message.outputText) {
            llmTracker.updateOutputTokens(message.outputText);
        }
        llmTracker.completeInteraction();
    ...
    if (message.action === "getLLMStatistics") {
        ...
        return llmTracker.getStatistics()
            .then(stats => {
